...
def goto_position_target_local_ned(north, east, down):
    ...
    while True:
        kuzey = round(iha.location.local_frame.north)
        dogu = round(iha.location.local_frame.east)
        if kuzey >= north * 0.9 and kuzey <= north * 1.2 :
            print("istenilen konuma ulasildi >> Kuzey : {}" .format(iha.location.local_frame.north))
            break
        print("Kuzey (X): {} metre".format(str(round(kuzey,2))))
        iha.send_mavlink(msg)
        time.sleep(1)
...
def iha_hareket(x, y, z):
    ...
    while irtifa_kontrol < -z*0.9  and x_kontrol < x *0.9 and y_kontrol < y *0.9 :
...
iha_hareket(0,5,7)      # zdegerini pozitif ver fonksiyon icinde negatifleniyor
time.sleep(1)
